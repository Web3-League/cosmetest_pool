    ...
    const fetchVolontaire = async () => {
      ...
      try {
        ...
        try {
          ...
          console.log("Détails du volontaire récupérés:", detailsData);
        } catch (detailsError) {
          console.warn(
        ...
        try {
          ...
          console.log("InfoBank du volontaire récupéré:", infoBankData);
        } catch (infoBankError) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        // Avec Axios, l'ID est dans response.data
        const newVolontaireId = response.data.id || response.data.idVol || response.data.volontaireId;

        setFormSuccess("Volontaire créé avec succès");

        // Redirection vers la page des détails du nouveau volontaire
        setTimeout(() => {
          navigate(`/volontaires/${newVolontaireId}`);
        }, 1500);
      }

      // Sauvegarder les informations bancaires séparément
      await saveBankInfo(volontaireId);

      // Redirection si création
      if (!isEditMode) {
        setTimeout(() => {
