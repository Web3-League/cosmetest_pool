...
const etudeService = {
  ...
  updatePayeStatus: async (idEtude, payeStatus) => {
    try {
      console.log(`🔄 Mise à jour statut PAYE étude ${idEtude} vers ${payeStatus}`);
  ...
  // ✅ NOUVELLE MÉTHODE : Vérifier et mettre à jour automatiquement le statut PAYE
  checkAndUpdatePayeStatus: async (idEtude, paiements) => {
    try {
      if (!paiements || paiements.length === 0) {
        console.log(`ℹ️ Aucun paiement pour l'étude ${idEtude}, statut PAYE = 0`);
      ...
      console.log(`📊 Étude ${idEtude}: ${paiements.length} paiements, tous payés: ${allPaid}, nouveau statut: ${newPayeStatus}`);
